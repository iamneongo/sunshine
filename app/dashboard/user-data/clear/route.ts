...
import { CLEAR_USER_DATA_CONFIRM_PHRASE, clearUserData, getUserDataAudit } from "@/lib/test-data";
...
function buildRedirect(request: Request, returnPath: string, status: string, extra: Record<string, string> = {}) {
  ...
  url.searchParams.set("userDataCleanup", status);
...
export async function POST(request: Request) {
  ...
  const audit = await getUserDataAudit();
  ...
  if (confirmPhrase !== CLEAR_USER_DATA_CONFIRM_PHRASE) {
  ...
  if (!Number.isFinite(confirmTotal) || confirmTotal !== audit.totalCount) {
    return buildRedirect(request, returnPath, "invalid_total", {
      expectedUserTotal: String(audit.totalCount)
    });
  }

  try {
    const result = await clearUserData(audit);
    return buildRedirect(request, returnPath, "success", {
      clearedUserLeads: String(result.leadCount),
      clearedUserEvents: String(result.eventCount)
    });
  } catch (error) {
